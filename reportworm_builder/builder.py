...
from reportworm_builder.template import Template

class Builder():

    def __init__(self, config:dict, cache:dict = {}) -> None:
        self.config = config
        self.flags = {
            "mode": config['mode'] if 'mode' in config else 'dev',
        }
        self.cache = cache
        self.refs = {}
        self.vars = {}
        ...
        try:
            ...
            raise FileNotFoundError("[Builder] build.json not found")
        ...
        for ref, dir_name in build_config['refs'].items():
            ...
            if not dir_path.is_dir():
                raise FileNotFoundError(f"[Builder] ref '{dir_name}' not found, skipping")
    ...
    def build_steps(self, build_config):
        ...
        for step in steps:
            if step not in build_config:
                raise KeyError(f"[Builder] step '{step}' not found in build.json")
    ...
    def batch(self, step_data:dict):
        ...
        if batch_type == 'html':
            ...
            raise KeyError(f"Unknown batch_type '{batch_type}'")


    def build_html(self, step_data:dict) -> Template:
        ref = step_data['base-ref']
        if ref not in self.refs['tpl']:
            raise KeyError(f"[Builder] Could not find ref '{ref}'")
        ...
        for token_info in step_data['tokens']:
            ...
            if token_type == 'tpl':
                ...
                raise KeyError(f"[Template] Unknown token type '{token_type}'")
